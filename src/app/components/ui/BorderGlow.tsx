...
function buildGlowVars(glowColor: string, intensity: number) {
  ...
  for (let i = 0; i < opacities.length; i++) {
    (vars as any)[`--glow-color${keys[i]}`] = `hsl(${base} / ${Math.min(opacities[i] * intensity, 100)}%)`;
  }
  return vars;
}
...
function buildGradientVars(colors: string[]) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    (vars as any)[GRADIENT_KEYS[i]] = `radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`;
  }
  (vars as any)['--gradient-base'] = `linear-gradient(${colors[0]} 0 100%)`;
  return vars;
}
